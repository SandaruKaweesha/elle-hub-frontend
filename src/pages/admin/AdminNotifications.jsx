...
import { Bell, CheckCircle2, Trophy, User, ShieldAlert, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto font-['Poppins'] space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Admin System Notifications</h1>
          <p className="text-xs md:text-sm text-gray-500 mt-1">Real-time alerts for registrations, tournament requests, and system activity.</p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-200 shadow-xs overflow-hidden flex flex-col min-h-[400px]">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 bg-gray-50/60">
          <div className="flex items-center gap-3">
            <span className="text-sm font-extrabold text-gray-900">All System Alerts</span>
        ...
        <div className="divide-y divide-gray-100 flex-1">
          {loading ? (
            <div className="py-20 text-center text-gray-400">
              ...
              <p className="text-xs font-bold">Loading Admin Notifications...</p>
            </div>
            ...
            <div className="py-20 text-center text-gray-400 space-y-2">
              ...
              <p className="text-xs text-gray-500">When users register or create tournaments, alerts will appear here.</p>
            </div>
          ) : (
            notifications.map(n => {
              ...
              return (
                ...
                >
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${
                    n.type === 'ACCOUNT' ? 'bg-blue-100 text-blue-700' :
                    n.type === 'TOURNAMENT' ? 'bg-amber-100 text-amber-700' :
                    'bg-gray-100 text-gray-700'
                  }`}>
                    {n.type === 'ACCOUNT' ? <User size={20} /> : <Trophy size={20} />}
                  </div>
        ...
        {!loading && notifications.length > 0 && (
          <div className="p-4 bg-gray-50 border-t border-gray-200 text-center">
            <p className="text-xs text-gray-500 font-semibold">End of admin notification stream.</p>
          </div>
        )}
